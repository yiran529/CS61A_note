class VendingMachine:
    ...
    def __init__(self,product,price):
        ...
        self.funds=0
    def vend(self):
        if self.product_num==0:return 'Inventory empty. Restocking required.'
        elif self.funds<self.price:
            return 'You must add ${0} more funds.'.format(self.price-self.funds)
        else:
            change=(' and ${0} change'.format(self.funds-self.price) if self.funds>self.price else '')
            self.funds=0
            self.product_num-=1
            return 'Here is your {0}{1}.'.format(self.product,change)
    def add_funds(self,new_funds):
        if self.product_num>0:
            self.funds+=new_funds
            return 'Current balance: ${0}'.format(self.funds)
        else:
            return 'Inventory empty. Restocking required. Here is your ${0}.'.format(new_funds)
...
def store_digits(n):
    ...
    class Link:
        ...
        def __init__(self,first,rest=()):
            assert isinstance(rest,Link) or rest==Link.empty,'Wrong Type!'
            self.first=first
            self.rest=rest
        def __repr__(self):
    ...
    def store(m):
        if m<10:
            ...
            power=num_of_digit(m)
            return Link(m//power,store(m%power))
    return store(n)
...
def preorder(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    if t.branches==[]:return[t.label]
    else:
        branches_pre=[]
        for branch in t.branches:
            branches_pre+=preorder(branch)
        return [t.label]+branches_pre
