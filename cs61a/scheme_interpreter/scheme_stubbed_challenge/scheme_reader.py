...
def scheme_read(src):
    ...
    # BEGIN PROBLEM 1/2
    "*** YOUR CODE HERE ***"
    pop_val=src.pop_first()
    if pop_val=='nil':return nil
    elif pop_val=='\'':return 'quote'
    elif pop_val=='true':return True
    elif pop_val=='false':return False
    elif pop_val=='(':return read_tail(src)
    elif pop_val==')':raise SyntaxError
    else: return pop_val
...
def read_tail(src):
    ...
    try:
        ...
        "*** YOUR CODE HERE ***"
        if src.current()==')':
            src.pop_first()#Don't forget this!
            return nil
        ...
        raise SyntaxError('unexpected end of file')
#print(read_tail ( Buffer(tokenize_lines(['1 2)']))))
