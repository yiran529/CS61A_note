...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        args = rest.map(lambda x: scheme_eval(x, env))#COOOOOOOOOOL
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):#what is this 'env' for?
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        signature,rest=expressions.first,expressions.rest
...
def do_and_form(expressions, env):
    ...
    if expressions==nil:return True#ATTENTION:DOOOOOOOON't return '#t'
    while expressions!=nil:
        val=scheme_eval(expressions.first,env)
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            # BEGIN PROBLEM 13
            perform=clause.rest
            if perform==nil:return test
            else:
                return eval_all(perform,env)
            "*** YOUR CODE HERE ***"
            # END PROBLEM 13
        expressions = expressions.rest
...
def make_let_frame(bindings, env):
    ...
    while bindings!=nil:
        ...
        name,value=bindings.first.first,bindings.first.rest.first
        value=scheme_eval(value,env)
        names=Pair(name,names)
        values=Pair(value,values)
        # if value in check:raise SchemeError
        # else: check[name]=value
